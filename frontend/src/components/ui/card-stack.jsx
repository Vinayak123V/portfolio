...
export function CardStack({
  ...
  // Stable autoplay using ref — not affected by stale closures
  const startInterval = React.useCallback(() => {
    if (intervalRef.current) clearInterval(intervalRef.current);
    if (!autoAdvance || !len) return;
    intervalRef.current = setInterval(() => {
      setActive((a) => (loop ? wrapIndex(a + 1, len) : a < len - 1 ? a + 1 : a));
    }, Math.max(500, intervalMs));
  }, [autoAdvance, intervalMs, len, loop]);

  // Start on mount, restart when deps change
  React.useEffect(() => {
  ...
  }, [startInterval, pauseOnHover, hovering]);
  ...
  return (
    <div className="relative h-full w-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-6">
        ...
        {item.description ? (
          <div className="mt-2 text-sm text-white/90 drop-shadow-md line-clamp-2">
            {item.description}
          </div>
        ) : null}
      </div>
